...
st.write(
    "Interactive EDA and an NLP-based search over Amazon product titles "
...
# -------------------------------
# Sidebar Filters with step-by-step counts
# -------------------------------
...
rating_categories = ["All"] + sorted(
    df["Rating_Category"].dropna().unique().tolist()
)
...
# 9.5.1 Metrics Row (Three Columns)
...
with col_a:
    ...
    st.metric(
        ...
        f"${avg_price:.2f}" if final_count > 0 else "N/A",
    )
...
with col_b:
    ...
    st.metric(
        ...
        f"{avg_rating:.2f}" if final_count > 0 else "N/A",
    )
...
with col_c:
    ...
    st.metric(
        ...
        f"{total_reviews:,}" if final_count > 0 else "N/A",
    )

# 9.5.2 Two Charts Side-by-Side (Columns)
...
with col1:
    ...
    if final_count > 0:
        ...
        price_hist_df = (
            pd.DataFrame(
                {
                    "Price_Bin": (bin_edges[:-1] + bin_edges[1:]) / 2,
                    "Count": price_hist,
                }
            )
            .set_index("Price_Bin")
...
# 9.5.3 Summary Statistics (Container)
with st.container():
...
# 9.5.4 Full Filtered Dataset (Expander)
with st.expander("Show Filtered Dataset"):
    st.dataframe(filtered_df.reset_index(drop=True))

# -------------------------------
# NLP Search on Product Titles (Within Filtered Subset)
# -------------------------------
@st.cache_data
...
st.write(
    "Use a free-text query to search for products within the **currently filtered subset**. "
    "TF–IDF and cosine similarity are used to rank titles."
)
...
if run_search:
    if not user_query:
        ...
        if k <= 0:
            ...
            results = (
                filtered_df.iloc[top_idx_local][
